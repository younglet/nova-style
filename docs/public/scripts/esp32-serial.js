 ...
 *
 * 用途：在浏览器里经 Web Serial API 把 nova-style.min.css 等前端文件
 * 写到 ESP32 的 /static/ 目录下。
 *
 * 项目特定说明（nova-style）：
 *   - 主要写 nova-style.min.css + index.novastyle.html
 *   - HTML 文件名加 .novastyle. 前缀，避免多项目写到同一 ESP32 时
 ...
 *     await esp.connect();
 *     await esp.writeFile('/static/nova-style.min.css', bytes, pct => {});
 *     await esp.disconnect();
 *   </script>
 *
 * 修改记录：
 *   2026-06-21  从 ssd1306_view 复制到 nova-frontend/nova-style/scripts/，
 *               上方 banner 改为 nova-style 特定说明，源码不动。
 */
